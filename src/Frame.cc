...
//Copy Constructor
    ...
    Frame::Frame(const cv::Mat &imLeft, const cv::Mat &imRight, const double &timeStamp, ORBextractor *extractorLeft,
        ...
        // Frame ID
        // nNextId是一个从0开始的变量，每次调用这个函数都会加1，赋给mnId
        mnId = nNextId++;
    ...
    Frame::Frame(const cv::Mat &imGray, const double &timeStamp, ORBextractor *extractor, ORBVocabulary *voc,
        ...
        // 这里简单整理一下参数的由来
        // imGray、timeStamp
        // 首先在主函数的for循环中依次读取每一帧影像，每读取一次，调用System类的对象SLAM的成员函数TrackMonocular，将影像传给它，
        // 然后在TrackMonocular函数中调用了Tracking类的对象tracker(它是System类的成员变量)的GrabImageMonocular函数。
        // 在GrabImageMonocular函数中将影像作为参数传给Frame类的构造函数，并将Frame对象返回给成员变量mCurrentFrame
        ...
        // 将我们传入的文件路径传给了Tracking的构造函数。
        // 在Tracking的构造函数中依次对这些参数进行了读取并赋给了相应成员变量。
        ...
        // 根据路径读取了数据并赋给成员变量mpVocabulary并且将这个成员变量作为参数又传给了Tracking类的构造函数。
        // 而Tracking类的构造函数又将传入的参数赋给了它的成员变量mpORBVocabulary，
        // 然后在Tracking的成员函数GrabImageMonocular中作为参数被传入Frame的构造函数，也就到了这里。

        // Frame ID
        // nNextId是一个初值为0的累加变量，每调用一次Frame的构造函数就加1，并将结果赋给mnId
        mnId = nNextId++;
        ...
        // ORB extraction
        // 对传入的影像提取ORB特征，提取结果返回给了Frame的成员变量mvKeys, mDescriptors
        ExtractORB(0, imGray);
    ...
    void Frame::ExtractORB(int flag, const cv::Mat &im) {
        // 调用Frame的成员变量mpORBextractorLeft和mpORBextractorRight提取ORB特征点
        // 注意下这里的用法，在ORBExtractor中重载了括号运算符
        if (flag == 0)
