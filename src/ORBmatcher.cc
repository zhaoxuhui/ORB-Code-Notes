...
namespace ORB_SLAM2 {
    ...
    int ORBmatcher::SearchByBoW(KeyFrame *pKF, Frame &F, vector<MapPoint *> &vpMapPointMatches) {
        ...
        // 对于匹配的具体细节如果不感兴趣也可以不看，不影响大局，只要知道几个循环后就可以得到匹配点对就OK了
        ...
        while (KFit != KFend && Fit != Fend) {
            if (KFit->first == Fit->first) {
                ...
                for (size_t iKF = 0; iKF < vIndicesKF.size(); iKF++) {
                    ...
                    // 如果地图点是空或其它非正常情况，跳过本次循环
                    if (!pMP)
                        continue;
                    // 如果地图点不为空但是坏的，也跳过本次循环
                    if (pMP->isBad())
                        continue;
                    ...
                    // 拿着vpMapPointsKF中的每一个点计算它和当前帧中所有特征点描述子之间的距离，其实思路是非常简单的
    ...
    int ORBmatcher::SearchForInitialization(Frame &F1, Frame &F2, vector<cv::Point2f> &vbPrevMatched,
        ...
        // 新建一个长度和F2中特征点数量相同的vector，每个元素的初始值是int能取到的最大值
                ...
                if (bestDist < (float) bestDist2 * mfNNratio) {
                    ...
                    if (mbCheckOrientation) {
                        ...
                        if (bin == HISTO_LENGTH)
                            bin = 0;
                        // 如果它的条件返回错误，则终止程序执行
                        // 因为根据上面的rotHist的定义，它的长度为HISTO_LENGTH
                        // 如果索引超过它了，数组就越界了
                        assert(bin >= 0 && bin < HISTO_LENGTH);
